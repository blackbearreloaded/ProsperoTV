...
foreach ($header in $programHeaders) {
    if ($header.Type -eq 1 -and $header.Align -gt 1 -and
        ($header.Offset % $header.Align) -ne ($header.Address % $header.Align)) {
        $errors.Add("LOAD $($header.Index) offset/address are not congruent for its alignment.")
    }
