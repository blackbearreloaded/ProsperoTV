...
# Build ordinary ELF facades for authorized system modules missing from the
# public SDK. They are consumed only by the linker and native module writer.
build_system_link_stub() {
    local library=$1
    local source=$2
    local object="$build/obj/${library}_link_stub.o"
    local output="$build/import-stubs/${library}.so"
...
for stub in "$videodec2_stub" "$agc_stub" "$agc_driver_stub"; do
    builder_stub_args+=(--stub "$stub")
    link_imports+=("$stub")
done
...
printf 'Executable PT_LOAD: %s bytes\n' "$executable_size_hex"
